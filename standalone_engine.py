...
# PIVOT_LEFT        = 2
# PIVOT_RIGHT       = 2
# MIN_R2            = 0.9900             # ← was 0.996
# MIN_ABS_SLOPE     = 0.0               # ← was 1.5
# MAX_ABS_SLOPE     = 10.0              # ← was 16.5
# MAX_POINT_RESIDUAL= 1e4
# MIN_GAP           = 13                # ← was 6
# MAX_GAP           = 100
# MIN_TOTAL_SPAN    = 6
# MAX_TOTAL_SPAN    = 300
# MIN_PRICE_RANGE   = 0.0
# MAX_LOOKAHEAD     = 25                # ← was 13  (critical fix for wide patterns)
# MAX_CLOSE_VIOL    = 0.0               # ← closes cannot cross the trendline
# PADDING_CANDLES   = 30                # ← was 50

# # Pattern Rejection Thresholds — Min
# MIN_DIST_RATIO    = 1.0               # ← was 0.65
# MIN_MAX_GAP_RATIO = 1.0               # ← was 0.65

# # Pattern Rejection Thresholds — Max  (new: upper cap)
# MAX_DIST_RATIO    = 3.0
# MAX_MAX_GAP_RATIO = 3.0

# # Telegram Settings (via Environment Variables)
# TELEGRAM_BOT_TOKEN = os.getenv('TELEGRAM_BOT_TOKEN')
# TELEGRAM_CHAT_ID   = os.getenv('TELEGRAM_CHAT_ID')

# # State Management — prevents duplicate alerts within a session
# SENT_ALERTS = set()
...
# def fetch_latest_ohlcv_delta(symbol=SYMBOL, resolution=RESOLUTION,
#                               lookback_candles=LOOKBACK_CANDLES, save_path=DEFAULT_CSV):
#     end_time           = int(time.time())
#     resolution_seconds = int(resolution.replace("m", "")) * 60
#     start_time         = end_time - (lookback_candles * resolution_seconds)
...
#     df = df[["global_index", "timestamp", "datetime",
...
# def detect_three_point_trendlines(pivot_idx, wick_array, close_array, side):
#     """
#     Detects 3-point trendlines through pivot highs or lows.

#     Close-violation check is intentionally removed — candles are allowed
#     to intersect the anchor trendline. The count of intersecting closes
#     is stored in 'n_close_intersect' for optional post-detection filtering.
#     """
#     results       = []
#     pivot_idx     = np.asarray(pivot_idx, dtype=int)
#     n_pivots      = len(pivot_idx)
...
#             gap1, gap2, total_span = i2 - i1, i3 - i2, i3 - i1
#             if (not (MIN_GAP <= gap1 <= MAX_GAP) or
#                     not (MIN_GAP <= gap2 <= MAX_GAP) or
#                     not (MIN_TOTAL_SPAN <= total_span <= MAX_TOTAL_SPAN)):
#                 continue
...
#             # ── Close violation hard rejection ────────────────────────────
#             seg_idx    = np.arange(i1, i3 + 1, dtype=int)
#             line_vals  = slope * (seg_idx - i1) + intercept
#             closes_seg = close_array[seg_idx]
...
#             # ── Close intersection count (info only) ──────────────────────
#             if side == "low" and direction == "up":
#                 n_close_intersect = int(np.sum(closes_seg < line_vals))
#             elif side == "high" and direction == "down":
#                 n_close_intersect = int(np.sum(closes_seg > line_vals))
#             else:
#                 n_close_intersect = 0
...
#                 "direction":         direction,
#                 "n_close_intersect": n_close_intersect,
#             })
#     return results
...
#     counter_prices = high_arr[sweep_idx] if side == "low" else low_arr[sweep_idx]
#     anchor_vals    = np.array([anchor_line_value(tl, int(ci)) for ci in sweep_idx])
#     gaps           = np.abs(counter_prices - anchor_vals)
...
#     consistency_score = float(np.sum(np.diff(gaps) < 0) / (len(gaps) - 1)) if len(gaps) >= 2 else 0.0
...
#     compression_score = 0.4 * n_dist + 0.4 * n_maxgap + 0.2 * n_tl_slope
...
#                                        all_pivot_lows: np.ndarray) -> List[Dict]:
#     for tl in trendlines:
#         tl["compression"] = counter_pivot_audit(tl, df, all_pivot_highs, all_pivot_lows)
#     return trendlines
...
#         if len(vis_idx) > 0:
#             vis_dt = [df["datetime"].iloc[int(ci)] for ci in vis_idx]

#             fig.add_trace(go.Scatter(
#                 x=vis_dt, y=vis_counter,
#                 mode="lines", name="Counter price",
#                 line=dict(color=c_line_color, width=1.2), showlegend=True
#             ))
#             fig.add_trace(go.Scatter(
#                 x=vis_dt + vis_dt[::-1],
#                 y=np.concatenate([vis_counter, vis_anchor[::-1]]).tolist(),
#                 fill="toself", fillcolor=fill_color,
#                 line=dict(color="rgba(0,0,0,0)"),
#                 name="Gap Zone", showlegend=True, hoverinfo="skip"
#             ))
#             fig.add_trace(go.Scatter(
#                 x=vis_dt, y=vis_counter,
#                 mode="markers",
#                 marker=dict(size=4, color=dot_color, symbol="circle"),
#                 name="Counter candles", showlegend=True
#             ))
...
#                     mdt = df["datetime"].iloc[int(midx)]
#                     fig.add_trace(go.Scatter(
#                         x=[mdt], y=[cp], mode="markers",
#                         marker=dict(size=14, color=mcolor, symbol="star",
#                                     line=dict(color="black", width=1)),
#                         name=mlabel, showlegend=True
#                     ))
#                     fig.add_trace(go.Scatter(
#                         x=[mdt, mdt], y=[av, cp], mode="lines",
#                         line=dict(color=mcolor, width=2.5), showlegend=False
#                     ))
...
# def send_alert(pattern, df):
#     detection_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
#     n_intersect    = pattern.get("n_close_intersect", 0)
...
#         f"• Duration          : {pattern['total_span']} candles\n"
#         f"• Price Range       : {pattern['price_range']:.2f}\n"
#         f"• Close Intersects  : {n_intersect}\n"
#     )
#     comp = pattern.get("compression", {})
#     message += (
#         f"• Compression Score : {comp.get('compression_score', 0.0):.2f} [{comp.get('compression_grade', '-')}]\n"
...
#     try:
#         # 1. Fetch Latest Data
#         fetch_latest_ohlcv_delta(symbol=SYMBOL, resolution=RESOLUTION,
#                                   lookback_candles=LOOKBACK_CANDLES, save_path=DEFAULT_CSV)
...
#         high_array  = high_series.to_numpy(dtype=float)
#         low_array   = low_series.to_numpy(dtype=float)
...
#         # 5. Enrich with compression metadata
#         enrich_trendlines_with_compression(all_trends, df, high_pivots, low_pivots)

#         # 6. Direction filter — wedge / channel setups only
#         filtered_trends = [
#             t for t in all_trends
#             if (t["side"] == "high" and t["direction"] == "down")
#             or (t["side"] == "low"  and t["direction"] == "up")
...
#         # 7. Compression filter — min AND max thresholds on DistR and MaxGapR
#         filtered_trends = [
#             t for t in filtered_trends
#             if (
#                 MIN_DIST_RATIO
#                     <= t.get("compression", {}).get("dist_ratio", 0.0)
#                     <= MAX_DIST_RATIO
#                 and
#                 MIN_MAX_GAP_RATIO
#                     <= t.get("compression", {}).get("max_gap_ratio", 0.0)
#                     <= MAX_MAX_GAP_RATIO
#             )
#         ]

#         print(f"[{datetime.now().strftime('%H:%M:%S')}] {len(filtered_trends)} patterns after filters.")

#         # 8. Check for New Alerts
#         new_alerts_count = 0
#         for t in filtered_trends:
#             i1, i2, i3 = t["pivot_1_idx"], t["pivot_2_idx"], t["pivot_3_idx"]
#             ts1 = df.loc[i1, "timestamp"]
#             ts2 = df.loc[i2, "timestamp"]
#             ts3 = df.loc[i3, "timestamp"]
#             pattern_id = (ts1, ts2, ts3)

#             if pattern_id not in SENT_ALERTS:
#                 t["datetime_1"] = df.loc[i1, "datetime"]
#                 t["datetime_2"] = df.loc[i2, "datetime"]
#                 t["datetime_3"] = df.loc[i3, "datetime"]

#                 target_col = t["side"]
#                 t["price_1"] = df.loc[i1, target_col]
#                 t["price_2"] = df.loc[i2, target_col]
#                 t["price_3"] = df.loc[i3, target_col]
...
def plot_three_point_trendline_plotly(df: pd.DataFrame,
    ...
    KEY FIX for Telegram image export:
      All x-axis values are converted to plain strings via strftime().
      Kaleido (the static image renderer) cannot handle timezone-aware
      pandas Timestamps — it silently fails or produces a blank chart.
      Converting to strings bypasses this entirely and works for both
      interactive display and write_image() PNG export.

    Visual layers:
      • Candlestick chart
      • Anchor trendline (lime = support, red = resistance)
      • Yellow circles   : 3 anchor pivot points
      • Filled area      : gap zone between anchor line and counter prices
      • Counter line     : opposing candle highs/lows across sweep window
      • Max gap stars    : biggest swing in each half highlighted
    """
    i1   = pattern["pivot_1_idx"]
    i2   = pattern["pivot_2_idx"]
    i3   = pattern["pivot_3_idx"]
    ...
    # ── Convert datetimes to strings — Kaleido-safe ───────────────────────
    # df["datetime"] is tz-aware (Asia/Kolkata). Kaleido cannot render
    # tz-aware Timestamps in static PNG export. strftime to plain string
    # fixes the blank-image bug without affecting visual appearance.
    fmt = "%Y-%m-%d %H:%M"
    local_x = local_df["datetime"].dt.strftime(fmt).tolist()

    def dt_to_str(idx):
        """Convert single df row datetime to string."""
        return df["datetime"].iloc[int(idx)].strftime(fmt)
    ...
    if comp:
        ...
        if len(vis_idx) > 0:
            # Convert all sweep datetimes to strings — tz-safe
            vis_dt = [dt_to_str(ci) for ci in vis_idx]
        ...
        state = comp["energy_state"]
        flag  = "COMPRESSING" if comp["is_compressing"] else "not compressing"
        tight = " TIGHTEST" if comp["at_tightest"] else ""
        comp_str = (f" | Score={score:.2f} [{grade}]  {flag}{tight}<br>"
                    f"DistR={dr:.2f}  MaxGapR={mgr:.2f}  [{state}]")
    else:
        comp_str = ""
    ...
    fig.update_layout(
        title=(f"Pattern ({side.upper()})"
               f" | R2={pattern['r2']:.4f}  Slope={slope:.2f}{comp_str}"),
...
def send_alert(pattern, df):
    ...
    try:
        fig = plot_three_point_trendline_plotly(df, pattern)
        # Explicit dimensions + scale required for reliable Kaleido PNG export.
        # width/height match the layout so no clipping occurs.
        # scale=2 gives 2400×1400px — sharp on mobile Telegram.
        fig.write_image(
            image_filename,
            format="png",
            width=1200,
        ...
        print(f"[INFO] Chart saved: {image_filename}")
        send_telegram_alert(message, image_path=image_filename)
    except Exception as e:
        print(f"[ERROR] Chart generation failed: {type(e).__name__}: {e}")
        traceback.print_exc()
        # Fallback: send text-only alert so you still get notified
        send_telegram_alert(message)
